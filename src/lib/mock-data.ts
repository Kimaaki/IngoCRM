...
/* Layout (teus componentes existem em src/components/...) */
import Sidebar from "../components/layout/Sidebar";
import Header from "../components/layout/Header";
import EditDialog from "../components/dialogs/EditDialog";

/* Mocks — ATENÇÃO ao nome do arquivo: mock-data.ts (com hífen) */
import {
  mockUsers,
  mockLeads,
  mockOrders,
      ...
      <Card>
        ...
        <CardContent>
          <p className="text-sm text-gray-600">
            Aqui podem viver gráficos e relatórios (KPI’s, funil, etc.).
          </p>
        </CardContent>
      </Card>
    </div>
...
function LeadsContent() {
  ...
  async function fetchLeads() {
    ...
    if (error) {
      console.warn("Tabela leads indisponível — usando mocks.");
          ...
          {leads.map((lead) => (
            <Card key={lead.id} className="p-4 shadow-sm">
              <CardHeader>
                ...
                <Badge className="mt-2 capitalize">{lead.status}</Badge>
              </CardHeader>
              <CardContent>
                ...
                <div className="flex flex-wrap gap-2 mt-3">
                  {[
                    ...
                    <Button
                      ...
                      variant="outline"
                      size="sm"
                      onClick={() => updateStatus(lead.id, status as LeadStatus)}
                    >
                      {status}
                    </Button>
                  ))}
                ...
                {mockOrders.map((order) => {
                  ...
                  return (
                    <tr key={order.id} className="hover:bg-gray-50">
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">#{order.id.slice(-6)}</td>
...
function CallModeContent() {
  // mock simples a partir de leads
  const demo = (mockLeads as any[]).slice(0, 6).map((l, i) => ({
    id: `call_${i}`,
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <Button size="sm"><Plus className="h-4 w-4 mr-2" />New Call</Button>
      </div>
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <Button size="sm"><Plus className="h-4 w-4 mr-2" />New Call Center</Button>
      </div>
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <div className="flex space-x-2">
          ...
          <Button size="sm"><Plus className="h-4 w-4 mr-2" />New Shipment</Button>
        </div>
      </div>
                ...
                {mockShipments.map((shipment) => {
                  ...
                  return (
                    <tr key={shipment.id} className="hover:bg-gray-50">
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">#{shipment.id.slice(-6)}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm">#{shipment.order_id.slice(-6)}</td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm">{shipment.tracking_number || "-"}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm">{shipment.carrier || "-"}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{formatDate(shipment.created_at)}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-right">
                        <div className="flex items-center justify-end space-x-2">
                          <Button variant="ghost" size="sm"><Eye className="h-4 w-4" /></Button>
                          <Button variant="ghost" size="sm"><Edit className="h-4 w-4" /></Button>
        ...
        <Card className="border-orange-200 bg-orange-50">
          <CardHeader>
            <CardTitle className="text-orange-800 flex items-center">
              <AlertTriangle className="h-5 w-5 mr-2" />Low Stock Alerts
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-2">
                ...
                {mockProducts.map((product) => {
                  ...
                  return (
                    <tr key={product.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm">{formatCurrency(product.unit_price)}</td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      <div className="mt-1">
                        {client.tags.map((tag: string) => (
                          <Badge key={tag} variant="outline" className="mr-1 text-xs">{tag}</Badge>
                        ))}
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm">{client.total_orders}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">{formatCurrency(client.total_spent)}</td>
                    ...
                    <div className="mt-2 text-xs text-gray-500">
                      {Object.entries(log.changes).map(([field, change]) => (
                        <div key={field} className="flex items-center space-x-2">
                          ...
                          <span className="bg-red-100 text-red-800 px-1 rounded">{String((change as any).old)}</span>
                          <span>→</span>
                          <span className="bg-green-100 text-green-800 px-1 rounded">{String((change as any).new)}</span>
                        </div>
                      ))}
                    </div>
                  )}
...
/* Placeholders visíveis para os restantes módulos */
const AnalyticsContent   = () => <Card><CardHeader><CardTitle>Analytics</CardTitle></CardHeader><CardContent>Em breve gráficos e KPIs.</CardContent></Card>;
const MonitoringContent  = () => <Card><CardHeader><CardTitle>Monitoring</CardTitle></CardHeader><CardContent>Monitoração do sistema e filas.</CardContent></Card>;
const AffiliatesContent  = () => <Card><CardHeader><CardTitle>Affiliates</CardTitle></CardHeader><CardContent>Gestão de afiliados.</CardContent></Card>;
const ValidationRulesContent = () => <Card><CardHeader><CardTitle>Validation Rules</CardTitle></CardHeader><CardContent>Regras de validação de leads.</CardContent></Card>;
const TasksContent       = () => <Card><CardHeader><CardTitle>Tasks</CardTitle></CardHeader><CardContent>Gestão de tarefas.</CardContent></Card>;
const TicketsContent     = () => <Card><CardHeader><CardTitle>Tickets</CardTitle></CardHeader><CardContent>Atendimento e suporte.</CardContent></Card>;
...
export default function IngoCRM() {
  const [activeTab, setActiveTab] = useState<
    | "dashboard" | "leads" | "orders" | "call-mode" | "call-centers" | "shipments"
    | "warehouse" | "clients" | "analytics" | "history" | "monitoring"
    | "affiliates" | "validation-rules" | "tasks" | "tickets"
    | "contacts" | "accounts" | "reports" | "settings"
  >("dashboard");

  const [leadStats, setLeadStats] = useState<Record<string, number>>({
    total: 0, new: 0, processing: 0, approved: 0, rejected: 0,
    callback: 0, spam: 0, verification: 0, delivered: 0, returned: 0,
  });

  // Contadores com Supabase + fallback p/ mocks
  useEffect(() => {
    async function load() {
      const { data, error } = await supabase.from("leads").select("status");
      const rows = (error ? (mockLeads as any[]) : ((data as any[]) ?? []));
      const counts: Record<string, number> = {
        total: rows.length, new: 0, processing: 0, approved: 0, rejected: 0,
        callback: 0, spam: 0, verification: 0, delivered: 0, returned: 0,
      };
      rows.forEach((l) => {
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      ...
      <div className="flex-1 flex flex-col overflow-hidden lg:ml-0">
        ...
        {/* Navegação rápida (opcional; Sidebar também troca abas se quiseres) */}
        <div className="flex gap-2 p-4 overflow-x-auto bg-white border-b">
          {[
            ...
            <Button
              key={k}
              variant={activeTab === (k as any) ? "default" : "outline"}
              size="sm"
        ...
        <main className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          {activeTab === "contacts" && (
            <Card><CardHeader><CardTitle>Contacts</CardTitle></CardHeader><CardContent>Em breve…</CardContent></Card>
          )}
          {activeTab === "accounts" && (
            <Card><CardHeader><CardTitle>Accounts</CardTitle></CardHeader><CardContent>Em breve…</CardContent></Card>
          )}
          {activeTab === "reports" && (
            <Card><CardHeader><CardTitle>Reports</CardTitle></CardHeader><CardContent>Em breve…</CardContent></Card>
          )}
          {activeTab === "settings" && (
            <Card><CardHeader><CardTitle>Settings</CardTitle></CardHeader><CardContent>Em breve…</CardContent></Card>
          )}
        </main>
      </div>
